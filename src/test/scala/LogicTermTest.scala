...
class LogicTermTest extends FlatSpec with Matchers {
  ...
  "The list" should "be correctly filled" in {
    var builder: PredicateBuilder = PredicateBuilder("myPredicate") += 5 += LogicList.from(Seq("alfa", "beta", "gamma"))
    var predicate: Predicate = builder.create()
    assert(predicate.toString() == "myPredicate(5,[alfa,beta,gamma])")
    builder = PredicateBuilder("myPredicate") += 5 += LogicList("alfa", "beta", "gamma")
    predicate = builder.create()
    assert(predicate.toString() == "myPredicate(5,[alfa,beta,gamma])")
  }
